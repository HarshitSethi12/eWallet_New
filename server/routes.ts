...
const liquidityRewards = new Map<string, LiquidityReward>();

// POST /api/rewards/calculate - Calculate liquidity mining rewards
router.post("/api/rewards/calculate", authenticateUser, async (req, res) => {
  ...
  try {
    ...
    // Calculate rewards based on liquidity provided and time
    const userReward = liquidityRewards.get(`${userId}-${poolId}`);
    ...
    // Reward calculation: 0.1% daily APR on liquidity provided
    const timeDiff = Date.now() - userReward.lastClaimTime.getTime();
    const daysPassed = timeDiff / (1000 * 60 * 60 * 24);
    const dailyRewardRate = 0.001; // 0.1% daily
    
    const newRewards = userReward.shares * pool.priceA * dailyRewardRate * daysPassed;
    userReward.claimableRewards += newRewards;

    res.json({
      success: true,
      rewards: {
        claimable: userReward.claimableRewards,
        pending: newRewards,
        totalEarned: userReward.rewardTokens + userReward.claimableRewards,
        apr: dailyRewardRate * 365 * 100 // Annual APR
      }
    });

  } catch (error) {
    console.error('Rewards calculation error:', error);
    res.status(500).json({ success: false, error: normalizeError(error) });
  }
});

// POST /api/rewards/claim - Claim liquidity mining rewards
router.post("/api/rewards/claim", authenticateUser, async (req, res) => {
  const { poolId } = req.body;
  const userId = getUserId(req).toString();
  ...
  try {
    ...
    if (!userReward || userReward.claimableRewards <= 0) {
      return res.status(400).json({ 
        success: false, 
    ...
    const claimedAmount = userReward.claimableRewards;
    
    // Transfer rewards to user
    userReward.rewardTokens += claimedAmount;
    userReward.claimableRewards = 0;
    userReward.lastClaimTime = new Date();

    res.json({
      success: true,
      message: 'Rewards claimed successfully',
      amount: claimedAmount,
      totalRewards: userReward.rewardTokens
    });
...
router.post("/api/alerts/create", authenticateUser, async (req, res) => {
...
router.get("/api/alerts", authenticateUser, async (req, res) => {
  const userId = getUserId(req).toString();
...
router.delete("/api/alerts/:id", authenticateUser, async (req, res) => {
  const { id } = req.params;
  const userId = getUserId(req).toString();
...
const referralProgram = new Map<string, ReferralData>();

// GET /api/referral/code - Get user's referral code
router.get("/api/referral/code", authenticateUser, async (req, res) => {
  const userId = getUserId(req).toString();

  try {
    let referralData = referralProgram.get(userId);

    if (!referralData) {
      // Generate unique referral code
      const referralCode = `REF${userId.slice(0, 6).toUpperCase()}${Date.now().toString(36)}`;
      ...
      referralProgram.set(userId, referralData);
    }
    ...
    res.json({
      ...
      stats: {
        referredUsers: referralData.referredUsers.length,
...
router.post("/api/referral/apply", authenticateUser, async (req, res) => {
  const { referralCode } = req.body;
  const userId = getUserId(req).toString();

  try {
    // Find referrer by code
    const referrer = Array.from(referralProgram.values())
    ...
    // Add referred user and give rewards
    referrer.referredUsers.push(userId);
    referrer.pendingRewards += 10; // $10 reward per referral

    res.json({
      success: true,
      message: 'Referral applied! You both earned rewards!',
      yourReward: 5,
      referrerReward: 10
    });
